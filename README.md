...
- **A name now gets the case the rest of the file gives it.** Whisper writes *"Socrates"* in one window and
  *"socrates"* in the next, *"Maps of Meaning"* here and *"maps of meaning"* there; nothing in the audio
...
- Ordinary words are kept out of it: a word the file mostly writes in lower case is never touched (*will*,
  *mark*, *rose*), two capitalised sightings are needed before a word counts as a name, and **Title-Cased
  stretches are ignored when reading the file's habits** — otherwise one *"It's A Rebuke To Everything"*
  would license capitalising *rebuke* everywhere else. Measured on a real 50-minute transcript, this made
  ten corrections: *i → I*, *adam → Adam*, *eve → Eve*, *god → God*, and nothing else.
...
- **A capital with nothing in front of it is now always settled.** The repair acted on a very short pause
  ...
  was — which is where most of them fall. *"in that order She's grateful"*, *"she serves them And they
...
- **Titles and names are protected by a better test than the pause**: every capital in a title has another
  capital beside it (*"a chapter called The Divinity of Interest"*, *"Snow White"*), while a stray one has
